...
class GPT(nn.Module):
    ...
    def num_scaling_params(self):
        ...
        total = wte + unembed_proj + value_embeds + lm_head + transformer_matrices + scalars
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02, weight_decay=0.0, scalar_lr=0.5):
        ...
        assert len(list(self.parameters())) == len(matrix_params) + len(embedding_params) + len(unembed_proj_params) + len(lm_head_params) + len(value_embeds_params) + len(resid_params) + len(x0_params) + len(smear_params)
